...
class iec61850client():
    ...
    def printDataDirectory(con, doRef):
        ...
        if error.value != 0:
            logger.error(
                "could not get logical device list, error:%i" % error.value)

        if dataAttributes:
            dataAttribute = lib61850.LinkedList_getNext(dataAttributes)

            while dataAttribute:
                daName = ctypes.cast(lib61850.LinkedList_getData(
                    dataAttribute), ctypes.c_char_p).value.decode("utf-8")
    ...
    def discovery(con):
        ...
        error = lib61850.IedClientError()
        deviceList = lib61850.IedConnection_getLogicalDeviceList(
            con, ctypes.byref(error))
        ...
        if deviceList:
            ...
            while device:
                LD_name = ctypes.cast(lib61850.LinkedList_getData(
                    device), ctypes.c_char_p).value.decode("utf-8")
                tmodel[LD_name] = {}

                logicalNodes = lib61850.IedConnection_getLogicalDeviceDirectory(
                    con, ctypes.byref(error), LD_name)
                ...
                while logicalNode:
                    LN_name = ctypes.cast(lib61850.LinkedList_getData(
                        logicalNode), ctypes.c_char_p).value.decode("utf-8")
                    tmodel[LD_name][LN_name] = {}

                    # [LNobjects, error] = iec61850.IedConnection_getLogicalNodeVariables(con, LD_name+"/"+LN_name)
                    LNobjects = lib61850.IedConnection_getLogicalNodeDirectory(
                        con, ctypes.byref(error), LD_name+"/"+LN_name, lib61850.ACSI_CLASS_DATA_OBJECT)
                    if error.value != 0:  # ret becomes int if connection is lost
                    ...
                    while LNobject:
                        Do = ctypes.cast(lib61850.LinkedList_getData(
                            LNobject), ctypes.c_char_p).value.decode("utf-8")
                        tmodel[LD_name][LN_name][Do] = {}

                        doRef = LD_name+"/"+LN_name+"."+Do

                        tmodel[LD_name][LN_name][Do] = iec61850client.printDataDirectory(
                            con, doRef)

                        LNobject = lib61850.LinkedList_getNext(LNobject)
                    lib61850.LinkedList_destroy(LNobjects)

                    LNdss = lib61850.IedConnection_getLogicalNodeDirectory(
                        con, ctypes.byref(error), LD_name+"/"+LN_name, lib61850.ACSI_CLASS_DATA_SET)
                    if error.value != 0:  # ret becomes int if connection is lost
                        lib61850.LinkedList_destroy(logicalNodes)
                        lib61850.LinkedList_destroy(deviceList)
                        return tmodel
                    ...
                    while LNds:
                        ...
                        DSname = ctypes.cast(lib61850.LinkedList_getData(
                            LNds), ctypes.c_char_p).value.decode("utf-8")
                        tmodel[LD_name][LN_name][DSname] = {}

                        #
                        isDel = ctypes.c_bool(False)
                        dataSetMembers = lib61850.IedConnection_getDataSetDirectory(
                            con, ctypes.byref(error), LD_name+"/"+LN_name+"."+DSname, ctypes.byref(isDel))
                        if error.value != 0:  # ret becomes int if connection is lost
                        ...
                        dataSetMemberRef = lib61850.LinkedList_getNext(
                            dataSetMembers)

                        i = 0
                        while dataSetMemberRef:
                            dsRef = ctypes.cast(lib61850.LinkedList_getData(
                                dataSetMemberRef), ctypes.c_char_p).value.decode("utf-8")
                            DX = dsRef[:-4]
                            FC = dsRef[-3:-1]
                            tmodel[LD_name][LN_name][DSname][str(i)] = {}
                            tmodel[LD_name][LN_name][DSname][str(
                                i)]['reftype'] = "DX"
                            tmodel[LD_name][LN_name][DSname][str(
                                i)]['type'] = "reference"
                            tmodel[LD_name][LN_name][DSname][str(
                    ...
                    LNrpp = lib61850.IedConnection_getLogicalNodeDirectory(
                        con, ctypes.byref(error), LD_name+"/"+LN_name, lib61850.ACSI_CLASS_URCB)
                    if error.value != 0:  # ret becomes int if connection is lost
                        lib61850.LinkedList_destroy(logicalNodes)
                        lib61850.LinkedList_destroy(deviceList)
                        return tmodel
                    ...
                    while LNrp:
                        Rp = ctypes.cast(lib61850.LinkedList_getData(
                            LNrp), ctypes.c_char_p).value.decode("utf-8")
                        tmodel[LD_name][LN_name][Rp] = {}

                        doRef = LD_name+"/"+LN_name+"."+Rp

                        tmodel[LD_name][LN_name][Rp] = iec61850client.printDataDirectory(
                            con, doRef)

                        LNrp = lib61850.LinkedList_getNext(LNrp)
                    lib61850.LinkedList_destroy(LNrpp)

                    LNbrr = lib61850.IedConnection_getLogicalNodeDirectory(
                        con, ctypes.byref(error), LD_name+"/"+LN_name, lib61850.ACSI_CLASS_BRCB)
                    if error.value != 0:  # ret becomes int if connection is lost
                        lib61850.LinkedList_destroy(logicalNodes)
                        lib61850.LinkedList_destroy(deviceList)
                        return tmodel
                    ...
                    while LNbr:
                        Br = ctypes.cast(lib61850.LinkedList_getData(
                            LNbr), ctypes.c_char_p).value.decode("utf-8")
                        tmodel[LD_name][LN_name][Br] = {}

                        doRef = LD_name+"/"+LN_name+"."+Br

                        tmodel[LD_name][LN_name][Br] = iec61850client.printDataDirectory(
                            con, doRef)
    ...
    def writeValue(con, model, ref, value):
        ...
        error = lib61850.IedClientError()
        lib61850.IedConnection_writeObject(
            con, ctypes.byref(error), ref, fc, mmsvalue)
    ...
    def updateValueInModel(con, model, ref):
        ...
        # print("updateValueInModel: ", val, path)
        ...
        def update_recurse(con, submodel, path):
            ...
            if len(path) < 1:
                ...
                if submodel[path[0]] and 'reftype' in submodel[path[0]] and submodel[path[0]]['reftype'] == 'DA':
                    ...
                    if error.value == 0:
                        submodel[path[0]]['value'],  submodel[path[0]
                                                              ]['type'] = iec61850client.printValue(value)
                        lib61850.MmsValue_delete(value)
                        err = 0
                    else:
    ...
    def getRef(model, path):
        ...
        for i in range(len(path)):
            if not path[i] in mm:
                logger.error("cannot find node in model: %s in %s" %
                             (path[i], ref))
                return ref, mm
                ...
                if tupl in self.reporting and len(self.reporting[tupl]) > 0:
                    for refdata in self.reporting[tupl]:
                        ...
                        rcb = lib61850.IedConnection_getRCBValues(
                            con, ctypes.byref(error), refdata["RPT"], rcb)
                        RptId = lib61850.ClientReportControlBlock_getRptId(rcb)
                        lib61850.IedConnection_installReportHandler(
                            con, refdata["RPT"], RptId, refdata["cbh"], id(refdata["refdata"]))

                        lib61850.ClientReportControlBlock_setRptEna(rcb, True)
                        lib61850.ClientReportControlBlock_setGI(rcb, True)
                        lib61850.IedConnection_setRCBValues(con, ctypes.byref(
                            error), rcb, lib61850.RCB_ELEMENT_RPT_ENA | lib61850.RCB_ELEMENT_GI, False)
                        refdata["rcb"] = rcb
    ...
    def registerWriteValue(self, ref, value):
        ...
        if uri_ref.scheme != "iec61850":
            logger.error(
                "incorrect scheme, only iec61860 is supported, not %s" % uri_ref.scheme)
            return -1
        ...
        if err == 0:
            ...
            model, error = iec61850client.writeValue(
                con, model, uri_ref.path[1:], value)
            if error == 0:
                self.connections[tupl]['model'] = model
                submodel, path = iec61850client.parseRef(
                    model, uri_ref.path[1:])  # get value from model via ref
                ...
                logger.error("could not write '%s' to %s with error: %i" %
                             (str(value), ref, error))
    ...
    def ReadValue(self, ref):
        ...
        if err == 0:
            ...
            if submodel:  # ref exists in model
                model, error = iec61850client.updateValueInModel(
                    con, model, uri_ref.path[1:])
                if error == 0:
                    self.connections[tupl]['model'] = model
                    submodel, path = iec61850client.parseRef(
                        model, uri_ref.path[1:])
                    logger.debug("Value '%s' read from %s" %
                                 (str(submodel), ref))
                    ...
                    logger.error(
                        "could not read '%s' with error: %i" % (ref, error))
                    if error == 3:  # we lost the connection
                        lib61850.IedConnection_destroy(con)
                        self.connections[tupl]['con'] = None
            else:
    ...
    def ReportHandler_cb(self, param, report):
        ...
        for index in dataset:
            reason = lib61850.ClientReport_getReasonForInclusion(
                report, int(index))
            if reason != lib61850.IEC61850_REASON_NOT_INCLUDED:
                mmsval = lib61850.MmsValue_getElement(
                    dataSetValues, int(index))
                DaRef = dataset[index]['value']
                val, _type = iec61850client.printValue(mmsval)
                logger.debug(DaRef + ":" + val + "(" + _type + ")")

                submodel, _ = iec61850client.parseRef(
                    self.connections[tupl]['model'], DaRef)
                submodel["value"] = val
    ...
    def registerForReporting(self, key, tupl, ref):
                        ...
                        if ("0" in model[LD_name][LN_name][DSname] and
                            ...
                            for index in model[LD_name][LN_name][DSname]:
                                if ref.startswith(model[LD_name][LN_name][DSname][index]['value']):
                                    logger.info("DATASET found! Ref:%s in DSref: %s" % (
                                        ref, model[LD_name][LN_name][DSname][index]['value']))
                                    logger.info(
                                        "  DSRef:%s" % LD_name + "/" + LN_name + "." + DSname)
                                    LD = LD_name
                                    LN = LN_name
                                    DS = DSname
                                    Idx = index
        ...
        for RP in model[LD][LN]:
            if "DatSet" in model[LD][LN][RP] and model[LD][LN][RP]["DatSet"]["value"] == LD + "/" + LN + "$" + DS:
                ...
                RPT = LD + "/" + LN + "." + \
                    model[LD][LN][RP]["DatSet"]["FC"] + "." + RP

                error = lib61850.IedClientError()
                if RPT in self.cb_refs:
                    logger.info("RPT allready registered")
                    rcb = lib61850.IedConnection_getRCBValues(
                        con, ctypes.byref(error), RPT, None)
                ...
                # model[LD][LN][DS])# bytes(str(LD + "/" + LN + "." + DS).encode('utf-8')) # ctypes.c_char_p( ref )
                ref = id(refdata)
                lib61850.IedConnection_installReportHandler(
                    con, RPT, RptId, cbh, ref)
                ...
                lib61850.ClientReportControlBlock_setGI(rcb, True)
                lib61850.IedConnection_setRCBValues(con, ctypes.byref(
                    error), rcb, lib61850.RCB_ELEMENT_RPT_ENA | lib61850.RCB_ELEMENT_GI, True)
                ...
                RcbData = {}
                RcbData["rcb"] = rcb
                # hard reference to ensure this pointer is not cleaned by the garbage collector
                RcbData["cbh"] = cbh
    ...
    def registerReadValue(self, ref):
        ...
        if err == 0:
            ...
            logger.error("no connection to IED: %s:%s, ref:%s not registered" % (
                uri_ref.hostname, port, ref))
        return -1
    ...
    def poll(self):
        for key in self.polling:
            ...
            if err == 0:
                ...
                model = self.connections[tupl]['model']
                if con and model:
                    model, err = iec61850client.updateValueInModel(
                        con, model, uri_ref.path[1:])
                    # print(uri_ref.path[1:])
                    if err == 0:
                        self.connections[tupl]['model'] = model
                        submodel, path = iec61850client.parseRef(
                            model, uri_ref.path[1:])
                        logger.debug("value:%s read from key: %s" %
                                     (str(submodel), key))
                        ...
                        logger.error(
                            "model not updated for %s with error: %i" % (key, err))
                ...
                logger.debug(
                    "IED not available for %s with error: %i" % (key, err))
    ...
    def get_controlObject(self, tupl, uri_ref):
        ...
        if not uri_ref.path[1:] in self.connections[tupl]['control'] or self.connections[tupl]['control'][uri_ref.path[1:]] == None:
            ...
            control = lib61850.ControlObjectClient_create(
                uri_ref.path[1:], con)
            ...
            if ctlModel == lib61850.CONTROL_MODEL_DIRECT_ENHANCED or ctlModel == lib61850.CONTROL_MODEL_SBO_ENHANCED:
                ...
                cbh = lib61850.CommandTerminationHandler(
                    self.commandTerminationHandler_cb)

                ref = bytes(uri_ref.path[1:].encode('utf-8'))

                lib61850.ControlObjectClient_setCommandTerminationHandler(
                    control, cbh, ctypes.c_char_p(ref))
                # hard reference to ensure this pointer is not cleaned by the garbage collector
                self.cb_refs.append(cbh)
        ...
        if err == 0:
            ...
            if error == 1:
                ...
                lastApplError = lib61850.ControlObjectClient_getLastApplError(
                    control)
                addCause = AddCause(lastApplError.addCause).name
                logger.info("LastApplError: %i, addCause: %s" %
            ...
            if ctlModel == lib61850.CONTROL_MODEL_SBO_NORMAL:
                ...
                if error == 1:
                    ...
                    lastApplError = lib61850.ControlObjectClient_getLastApplError(
                        control)
                    addCause = AddCause(lastApplError.addCause).name
                    logger.error("LastApplError: %i, addCause: %s" %
                                 (lastApplError.error, addCause))
                ...
                ctlVal = iec61850client.getMMsValue("", value, 0, mmsType)
                lib61850.ControlObjectClient_setOrigin(control, "mmi", 3)
                error = lib61850.ControlObjectClient_selectWithValue(
                    control, ctlVal)
                if error == 1:
                    logger.info("select: %s returned succesfull" % value)
                else:
                ...
                logger.error(
                    "cannot select object with ctlmodel: %i" % ctlModel)
                addCause = (
                    "cannot select object with ctlmodel: %i" % ctlModel)
                error = -1
...
#     tcpPort = 102
    ...
    # cl = iec61850client()
    # model = cl.getDatamodel(None, hostname, tcpPort)
    # # print(model)
    # err = cl.registerReadValue(
    #     "iec61850://127.0.0.1:102/TEMPLATEMET/METMMXU1.Hz.instMag.f")
    # err = cl.registerReadValue(
    #     "iec61850://127.0.0.1:102/TEMPLATEMET/METMMXU1.TotPF.instMag.f")
    # while True:

    #     cl.poll()
    # cl.printrefs(model)
    # err = cl.registerReadValue("TRAFO_3Cashel/MMXU1.PhV.phsA.instCVal.mag.f")
    # print(err)


#
#     max_client = len(tcpPort)
#     error = [None]*max_client
#     con = [None]*max_client
#     model = [None]*max_client

#     for i in range(max_client):
#         error[i] = lib61850.IedClientError()
#         con[i] = lib61850.IedConnection_create()

#     for i in range(max_client):
#         lib61850.IedConnection_connect(
#             con[i], ctypes.byref(error[i]), hostname[i], tcpPort[i])
#         # model[i] = iec61850client.discovery(con[i])

    # iec61850client.registerReadValue("TEMPLATEMET/METMMXU1.A.phsA.instCVal.mag.f")
#     # value = lib61850.IedConnection_readObject(con[0], ctypes.byref(
#     #     error[0]), "TEMPLATEMET/METMMXU1.A.phsA.instCVal.mag.f", lib61850.IEC61850_FC_MX)
...
#         time.sleep(1)
#         value = lib61850.IedConnection_readObject(con[0], ctypes.byref(
#             error[0]), "TEMPLATEMET/METMMXU1.Hz", lib61850.IEC61850_FC_MX)
#         # val, types = iec61850client.printValue(value)
#         for arr in range(lib61850.MmsValue_getArraySize(value)):
#             # val = lib61850.MmsValue_getElement(value, 0)
#             val, types = iec61850client.printValue(
#                 lib61850.MmsValue_getElement(value, arr))
#             if types == 'structure':
#                 for a in range(lib61850.MmsValue_getArraySize(val)):
#                     val2, types2 = iec61850client.printValue(
#                         lib61850.MmsValue_getElement(val, a))
#                     print(types2)
#                     if types2 == 'structure':
#                         for b in range(lib61850.MmsValue_getArraySize(val2)):
#                             val3, types3 = iec61850client.printValue(
#                                 lib61850.MmsValue_getElement(val2, b))
#                             # print(val3, types3)
#             # print(val, types)
#         lib61850.MmsValue_delete(value)
#         # print(val)
#         # time.sleep(1)
#         # values = lib61850.IedConnection_readObject(con[1], ctypes.byref(
#         #     error[0]), "TEMPLATEMET/METMMXU1.A.phsA.instCVal.mag.f", lib61850.IEC61850_FC_MX)
#         # val, types = iec61850client.printValue(values)
#         # print(val)
#         # lib61850.MmsValue_delete(values)
#         i += 1
#         if i > 5:
#             break
    # error = lib61850.IedClientError()
    # con = lib61850.IedConnection_create()
    # lib61850.IedConnection_connect(
    #     con, ctypes.byref(error), hostname, tcpPort[0])
    # if (error.value == lib61850.IED_ERROR_OK):
    ...
    # cl = [iec61850client()]*3
    # model = [a.getDatamodel(None, hostname, 102) for a in cl]
    # print(model)
    # [a.printrefs(None, hostname, 102) for a in model]
